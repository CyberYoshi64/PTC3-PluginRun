...
/// Jump to HOME Menu (with optional arguments)
typedef enum {
	APTHOME_NONE = 0,	///< Jump to HOME Menu without additional operations
	APTHOME_EMANUAL,	///< Launch this application's instruction manual
	APTHOME_THEMESHOP,	///< Launch Theme Shop (broken or missing arguments?)
	APTHOME_BADGEBOX,	///< Open the badge box
} APT_HomeMenuArgType;

typedef enum {
	APTCAPTURE_NONE = 0,		///< No setting specified. (Let Miiverse decide)
	APTCAPTURE_NONE_OLD_SDK,	///< No setting for old SDK (Set automatically, then set manually in newer SDK games).
	APTCAPTURE_ENABLED,			///< Both screens can be uploaded.
	APTCAPTURE_DISABLED,		///< None of the screens can be uploaded.
	APTCAPTURE_TOP_ONLY,		///< Only the top screen can be uploaded.
	APTCAPTURE_BOT_ONLY,		///< Only the bottom screen can be uploaded.
} aptScreenCapPermission;
...
Result aptJumpToHomeMenu(void);

/**
 * @brief Jumps back to the HOME Menu with arguments.
 * 
 * @param buf Input buffer that contains parameters on entry and result data on exit.
 * @param bufsize Size of the buffer.
 * @param handle Handle to pass to the HOME Menu.
 */
Result aptJumpToHomeMenuArg(void* buf, size_t bufsize, Handle handle);

/// Jumps to the Instruction Manual of the current application.
/// @note When the manual is closed, HOME Menu immediately resumes the application.
Result aptJumpToEManual(void);

/// Jumps to the Theme Shop.
/// @note This feature is either broken or missing arguments.
Result aptJumpToThemeShop(void);

/// Jumps to the HOME Menu to open the badge box automatically.
Result aptJumpToBadgeBox(void);
 ...
 */
Result aptLaunchLibraryApplet(NS_APPID appId, void* buf, size_t bufsize, Handle handle);
/**
 ...
 */
Result aptLaunchSystemApplet(NS_APPID appId, void* buf, size_t bufsize, Handle handle);
...
/**
 * @brief Get permission state to post screenshots on Miiverse.
 * 
 * @param permissionType Output Permission Setting (refer to @ref aptScreenCapPermission)
 */
Result APT_GetScreenCapturePostPermission(u32* permissionType);

/**
 * @brief Set permission state to post screenshots on Miiverse.
 * 
 * @param permissionType Permission Setting (refer to @ref aptScreenCapPermission)
 */
Result APT_SetScreenCapturePostPermission(u32 permissionType);
