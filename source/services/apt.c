...
Result aptJumpToHomeMenu(void)
{
	...
	if R_FAILED(res = APT_PrepareToJumpToHomeMenu()) {
	...
	res = APT_JumpToHomeMenu(NULL, 0, 0);
	aptFlags &= ~FLAG_ACTIVE;
...
Result aptJumpToHomeMenuArg(void* buf, size_t bufsize, Handle handle)
{
	...
	res = APT_JumpToHomeMenu(buf, bufsize, handle);
	aptFlags &= ~FLAG_ACTIVE;
...
Result aptJumpToEManual(void) {
	u32 param[2] = {0};
	memcpy((void*)param, "ASHP", 4);
	param[1] = APTHOME_EMANUAL;
	return aptJumpToHomeMenuArg(param, 5, 0);
}

Result aptJumpToThemeShop(void) {
	u32 param[2] = {0};
	memcpy((void*)param, "ASHP", 4);
	param[1] = APTHOME_THEMESHOP;
	return aptJumpToHomeMenuArg(param, 5, 0);
}
...
Result aptLaunchLibraryApplet(NS_APPID appId, void* buf, size_t bufsize, Handle handle)
{
	...
	if R_FAILED(res = APT_PrepareToStartLibraryApplet(appId)) {
		aptSetSleepAllowed(sleep);
		return res;
	}
	aptSetSleepAllowed(sleep);
	...
	aptSetSleepAllowed(false);
	res = APT_StartLibraryApplet(appId, buf, bufsize, handle);
	aptFlags &= ~FLAG_ACTIVE;
...
Result aptLaunchSystemApplet(NS_APPID appId, void* buf, size_t bufsize, Handle handle)
{
	...
	aptSetSleepAllowed(false);
	aptFlags &= ~FLAG_SPURIOUS; // If we haven't received a spurious wakeup by now, we probably never will (see aptInit)
	if R_FAILED(res = APT_PrepareToStartSystemApplet(appId)) {
		aptSetSleepAllowed(sleep);
		return res;
	}
	...
	res = APT_StartSystemApplet(appId, buf, bufsize, handle);
	aptFlags &= ~FLAG_ACTIVE;
...
{
	u32 cmdbuf[16];
	cmdbuf[0] = IPC_MakeHeader(0x56,0,0); // 0x00560000

	Result ret = aptSendCommand(cmdbuf);
	if (R_SUCCEEDED(ret))
	{
		if(permissionType)	*permissionType = cmdbuf[2];
	}
...
{
	u32 cmdbuf[16];
	cmdbuf[0] = IPC_MakeHeader(0x55,1,0); // 0x00550040
	cmdbuf[1] = permissionType;
